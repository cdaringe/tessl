...
},{"./isObject":"../node_modules/lodash/isObject.js","./now":"../node_modules/lodash/now.js","./toNumber":"../node_modules/lodash/toNumber.js"}],"point-maths.ts":[function(require,module,exports) {
"use strict";
...
var path_edit_1 = require("./path-edit");
...
function distanceRectilinear(p1, p2) {
  var dx = p2[0] - p1[0];
  var dy = p2[1] - p1[1];
  return Math.abs(dx) + Math.abs(dy);
}

var isNear = function isNear(p1, p2) {
  var px = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  return distanceRectilinear(p1, p2) < px;
};
/**
...
function insertPointBetweenBounds(_ref) {
  var initialPathLength = _ref.initialPathLength,
      pathNode = _ref.pathNode,
      pathPoints = _ref.pathPoints,
      selectedPoint = _ref.selectedPoint;
  if (!pathPoints.length || pathPoints.length < 2) throw new Error('not enough points');
  var derivedPoint = pathNode.getPointAtLength(initialPathLength);

  if (!isNear(selectedPoint, [derivedPoint.x, derivedPoint.y])) {
    throw new Error(['initialPathLength does not produce initial point', "([".concat(derivedPoint.x, ", ").concat(derivedPoint.y, "]) equivalent"), "passed point (".concat(selectedPoint.toString(), ")")].join(' '));
  }

  var currentLength = initialPathLength;

  var _loop = function _loop() {
    var _pathNode$getPointAtL = pathNode.getPointAtLength(currentLength),
        x = _pathNode$getPointAtL.x,
        y = _pathNode$getPointAtL.y;

    var distancesToPoint = pathPoints.map(function (mp) {
      return distance(mp.point, [x, y]);
    });
    var minTravelToNextPoint = Math.min.apply(Math, _toConsumableArray(distancesToPoint));

    if (minTravelToNextPoint < 1) {
      var i = distancesToPoint.findIndex(function (d) {
        return d === minTravelToNextPoint;
      });

      var nextPathPoints = _toConsumableArray(pathPoints);

      var nexPoint = {
        point: selectedPoint,
        ref: path_edit_1.toPointRef(selectedPoint)
      };
...
exports.insertPointBetweenBounds = insertPointBetweenBounds;

function findClosest(pathNode, point) {
  /* eslint-disable */
  var pathLength = pathNode.getTotalLength();
  var precision = 8;
  var bestDistance = Infinity;
  var best = new DOMPoint(0, 0, 0, 0);
  var bestLength = 0; // linear scan for coarse approximation
...
function createMouseMoveHandler(_ref2) {
  var dispatch = _ref2.dispatch,
      path = _ref2.path,
      snapperLine = _ref2.snapperLine,
      snapperDot = _ref2.snapperDot,
      state = _ref2.state;

  var setSnapperModeActive = function setSnapperModeActive(isNodeAddEnabled) {
  ...
  var updateSnapper = debounce_1.default(function updateSnapper(mousePoint) {
    var _findClosest = findClosest(path, mousePoint),
        distance = _findClosest.distance,
        point = _findClosest.point,
        length = _findClosest.pathLength;

    if (distance < 15 || distance > 100) return setSnapperModeActive(false);
    setSnapperModeActive(true);
    snapperLine.attr('x1', point[0]).attr('y1', point[1]).attr('x2', mousePoint[0]).attr('y2', mousePoint[1]);
    snapperDot.attr('cx', point[0]).attr('cy', point[1]).lower();
    dispatch({
...
},{"d3":"../node_modules/d3/index.js","lodash/debounce":"../node_modules/lodash/debounce.js","./path-edit":"path-edit.ts"}],"path-edit.ts":[function(require,module,exports) {
"use strict";
...
exports.toPointRef = function (_ref) {
  ...
  return "".concat(x.toFixed(3), "_").concat(y.toFixed(3));
};

exports.createNodeSnapper = function (_ref3) {
  var dispatch = _ref3.dispatch,
      onAddNode = _ref3.onAddNode,
      getPath$ = _ref3.getPath$,
      state = _ref3.state,
      svg$ = _ref3.svg$;
  svg$.on('mousemove', point_maths_1.createMouseMoveHandler({
    dispatch: dispatch,
    path: getPath$().node(),
    snapperDot: exports.appendCircle({
      x: -10,
      y: -10,
      node$: svg$
    }),
...
exports.appendCircle = function (_ref4) {
  var x = _ref4.x,
      y = _ref4.y,
      r = _ref4.r,
      node$ = _ref4.node$;
  return node$.append('circle').attr('cx', x).attr('cy', y).attr('r', r || 10);
};

exports.appendPathToSvg = function (_ref5) {
  var points = _ref5.points,
      svg$ = _ref5.svg$;
  return svg$.append('path').attr('d', d3.line().curve(d3.curveCatmullRom.alpha(0.5))(points)).attr('stroke', 'black').attr('fill', 'none');
};

exports.createSvg = function (width, height) {
  return d3.select('body').append('svg').attr('width', width).attr('height', height).node();
};

exports.createNodeDragger = function (_ref6) {
  var circle$ = _ref6.circle$,
      onDrag = _ref6.onDrag,
      onDragEnd = _ref6.onDragEnd,
      onDragStart = _ref6.onDragStart,
      pointIndex = _ref6.pointIndex;
  ;
...
exports.appendPath = function (_ref7) {
  var bindUndo = _ref7.bindUndo,
      canEditPoint = _ref7.canEditPoint,
      onStateChange = _ref7.onStateChange,
      points = _ref7.points,
      svg$ = _ref7.svg$;
  var state = {
    history: [],
    ...
    path$: null,
    metaPoints: points.map(function (point) {
      var _point = _slicedToArray(point, 2),
          x = _point[0],
          y = _point[1];

      return {
        circle$: exports.appendCircle({
          x: x,
          y: y,
          node$: svg$
        }),
        point: point,
        ref: exports.toPointRef(point)
      };
    }),
    svg$: svg$
  };
    ...
    var updateState = function updateState() {
      ...
      switch (msg.type) {
        case 'AppendPointCircles':
          {
            state.metaPoints.forEach(function (mp) {
              if (mp.circle$) return;

              var _mp$point = _slicedToArray(mp.point, 2),
                  x = _mp$point[0],
                  y = _mp$point[1];

              mp.circle$ = exports.appendCircle({
                x: x,
                y: y,
                node$: state.svg$
              });
        ...
        case 'BindNodeDragHandlers':
          {
            state.metaPoints.forEach(function (mp, i) {
              var circle$ = mp.circle$,
                  isDragHandlerBound = mp.isDragHandlerBound,
                  point = mp.point,
                  ref = mp.ref;
              if (isDragHandlerBound || !circle$) return;
              if (canEditPoint && !canEditPoint(state, point, i)) return;
              mp.isDragHandlerBound = true;
              exports.createNodeDragger({
                circle$: circle$,
                pointIndex: i,
                onDragStart: function onDragStart(pointIndex) {
                  return state.draggedNodeStartPoint = points[pointIndex];
                },
                ...
                onDragEnd: function onDragEnd(pointIndex) {
                  var metaPoint = state.metaPoints[pointIndex];
                  var currentPoint = metaPoint.point;
                  var startPoint = state.draggedNodeStartPoint;
                  delete state.draggedNodeStartPoint; // dangerously swap old point value in just for a hot second,
                  ...
                  metaPoint.point = currentPoint; // phew! sorry.  that was gross.
                }
              });
        ...
        case 'CreateNodeSnapper':
          {
            ...
            state.nodeSnapperUnsubscribe = exports.createNodeSnapper({
              dispatch: dispatch,
        ...
        case 'DrawPath':
          {
            ...
            state.path$ = exports.appendPathToSvg({
              points: state.metaPoints.map(function (m) {
                return m.point;
              }),
              svg$: state.svg$
            });
        ...
        case 'MoveNodePosition':
          {
            var _msg$value = msg.value,
                pointIndex = _msg$value.pointIndex,
                x = _msg$value.x,
                y = _msg$value.y;
            var movedPoint = state.metaPoints[pointIndex].point;
        ...
        case 'PromoteNodePlacementCandidatePoint':
          {
            ...
            var _state$nodePlacementC = state.nodePlacementCandidate,
                point = _state$nodePlacementC.point,
                length = _state$nodePlacementC.length;
            dispatch({
              type: 'DisableNodePlacement'
            });
            state.metaPoints = point_maths_1.insertPointBetweenBounds({
...
},{"./point-maths":"point-maths.ts","d3":"../node_modules/d3/index.js"}],"demo/undo-redo.ts":[function(require,module,exports) {
"use strict";
...
function check(ev, style, shift) {
  ...
  var winAllow = !style || style === 'windows';
...
},{}],"demo/handle-key-events.ts":[function(require,module,exports) {
"use strict";
...
exports.bindKeyEvents = function (_ref) {
...
},{"./undo-redo":"demo/undo-redo.ts"}],"demo-curvy-line.ts":[function(require,module,exports) {
"use strict";
...
var handle_key_events_1 = require("./demo/handle-key-events");

var d3 = require("d3");

var points = [[10, 80], [100, 100], [200, 30], [300, 50], [400, 40], [500, 80]];
var height = 600;
var width = 800;
var svg$ = d3.select(document.getElementById('demo')).attr('width', width).attr('height', height);
var undoEditNode = null;
var path$ = path_edit_1.appendPath({
  canEditPoint: function canEditPoint(state, _, i) {
    return !(i === 0 || state.metaPoints.length - 1 == i);
  },
  onStateChange: function onStateChange(state) {// onStateChange(state)
  },
  bindUndo: function bindUndo(dispatchUndo) {
...
},{"./demo.css":"demo.css","./path-edit":"path-edit.ts","./demo/handle-key-events":"demo/handle-key-events.ts","d3":"../node_modules/d3/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","demo-curvy-line.ts"], null)
//# sourceMappingURL=/demo-curvy-line.dcaa750e.js.map