...
exports.render = ({ nodes, onDisableEditing, onEnableEditing, onShowNodes, onToggleLineMode, snapperState }) => {
    ...
    react_dom_1.default.render(<div>
      ...
      <div id='state'>
        ...
        <ul>
          {nodes.map((node, i) => (<li key={i} children={<pre children={node.point.map(toFourChars).join(', ')}/>}/>))}
        </ul>
      </div>
    </div>, self.document.getElementById('controls'));
};
