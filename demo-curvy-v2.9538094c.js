...
exports.circle = {
  ...
  set: function set(_ref2) {
    var x = _ref2.x,
        ...
        r = _ref2.r,
        node$ = _ref2.node$;
    return node$.attr('cx', x).attr('cy', y).attr('r', r || 10);
  }
};
exports.path = {
  renderLine: function renderLine(path$, points) {
    return path$.attr('d', d3.line().curve(d3.curveCatmullRom.alpha(0.5))(points));
  }
};
},{"d3":"../node_modules/d3/index.js"}],"path-edit/dragger.ts":[function(require,module,exports) {
"use strict";
...
exports.createNodeDragger = function (_ref) {
  var node$ = _ref.node$,
      onDrag = _ref.onDrag,
...
},{"d3":"../node_modules/d3/index.js"}],"point-maths.ts":[function(require,module,exports) {
"use strict";
...
function getPointInsertionIndex(_ref) {
  var point = _ref.point,
      pathDistance = _ref.pathDistance,
      pathNode = _ref.pathNode,
      points = _ref.points;
  if (!points.length || points.length < 2) throw new Error('not enough points');
  var derivedPoint = pathNode.getPointAtLength(pathDistance);

  if (!isNear(point, [derivedPoint.x, derivedPoint.y])) {
    throw new Error(['pathDistance does not produce initial point', "([".concat(derivedPoint.x, ", ").concat(derivedPoint.y, "]) equivalent"), "passed point (".concat(point.toString(), ")")].join(' '));
  }

  var currentLength = pathDistance;
  ...
  var _loop = function _loop() {
    ...
    if (minTravelToNextPoint < 1) {
      return {
        v: distancesToPoint.findIndex(function (d) {
...
exports.getPointInsertionIndex = getPointInsertionIndex;
...
},{}],"path-edit/snapper.ts":[function(require,module,exports) {
"use strict";
...
exports.createNodeSnapper = function (_ref) {
  var onAddNode = _ref.onAddNode,
      path$ = _ref.path$,
      svg$ = _ref.svg$;
  var state = {
    pathPoint: null,
  ...
  var snapperDot = dom_1.circle.append({
    x: -10,
    y: -10,
    node$: svg$
  });
  snapperDot.attr('class', 'snapper');
  var snapperLine = svg$.append('line');

  var setSnapperUiVisible = function setSnapperUiVisible() {
    snapperLine.style('opacity', state.isNodeAddEnabled ? 1 : 0);
    snapperDot.style('opacity', state.isNodeAddEnabled ? 1 : 0);
  };

  var onMouseOverActiveRegion = function onMouseOverActiveRegion(pathPoint) {
  ...
  var onMouseOutActiveRegion = function onMouseOutActiveRegion() {
    if (!state.isNodeAddEnabled) return; // optimization
  ...
  svg$.on('mousemove', createMouseMoveHandler({
    onMouseOverActiveRegion: onMouseOverActiveRegion,
    onMouseOutActiveRegion: onMouseOutActiveRegion,
    path: path$.node(),
    snapperDot: snapperDot,
    snapperLine: snapperLine
  }));
  svg$.on('click', function () {
    var pathPoint = state.pathPoint,
        isNodeAddEnabled = state.isNodeAddEnabled;
...
function createMouseMoveHandler(_ref2) {
  var onMouseOverActiveRegion = _ref2.onMouseOverActiveRegion,
      onMouseOutActiveRegion = _ref2.onMouseOutActiveRegion,
      path = _ref2.path,
      snapperLine = _ref2.snapperLine,
      snapperDot = _ref2.snapperDot;
  var rendering = false;

  function updateSnapper(mousePoint) {
    // @info
    // optimize.  findClosest is expensive. queue requests, whilst
    // still accepting the _latest_ point for next paint
    var nextPoint = mousePoint;
    if (rendering) return;
    window.requestAnimationFrame(function getNextSnapperPoint() {
      rendering = true;

      var _point_maths_1$findCl = point_maths_1.findClosest(path, nextPoint),
          distance = _point_maths_1$findCl.distance,
          point = _point_maths_1$findCl.point,
          length = _point_maths_1$findCl.pathLength;
...
},{"./dom":"path-edit/dom.ts","../point-maths":"point-maths.ts","d3":"../node_modules/d3/index.js"}],"path-edit/mod.ts":[function(require,module,exports) {
"use strict";
...
exports.toMetaNodes = function (points) {
...
function renderNodes(opts) {
  var svg$ = opts.svg$,
      ...
      onAddHistory = opts.onAddHistory;
  nodes.forEach(function renderNode(mp, i) {
    var _mp$point = _slicedToArray(mp.point, 2),
        x = _mp$point[0],
        y = _mp$point[1];

    if (!mp.ref) mp.ref = exports.toPointRef(mp.point);

    if (!mp.node) {
      var node$ = dom_1.circle.append({
        x: x,
        y: y,
        node$: svg$
      });
      dragger_1.createNodeDragger({
        node$: node$,
        pointIndex: i,
        ...
        onDragEnd: function onDragEnd(pointIndex) {
          var node = nodes[pointIndex];
          var currentPoint = node.point; // dangerously swap old point value in just for a hot second,
          ...
          delete mp.node.dragStartPoint; // phew! sorry.  that was gross.
        }
      });
      mp.node = {
        node$: node$
  ...
  dom_1.path.renderLine(path$, nodes.map(function (node) {
    return node.point;
  }));
}
...
exports.fromPoints = function (opts) {
  var _opts$historySize = opts.historySize,
      historySize = _opts$historySize === void 0 ? 25 : _opts$historySize,
      points = opts.points,
      svg$ = opts.svg$;
  var nodes = exports.toMetaNodes(points);
  var path$ = svg$.append('path').attr('stroke', 'black').attr('fill', 'none');
  snapper_1.createNodeSnapper({
    onAddNode: function onAddNode(_ref4) {
      var length = _ref4.length,
          point = _ref4.point;
      var insertIndex = point_maths_1.getPointInsertionIndex({
        pathDistance: length,
        pathNode: path$.node(),
        point: point,
  ...
  var onAddHistory = function onAddHistory() {
    if (historySize === 0) return;
    history.push(JSON.stringify(nodes.map(function (node) {
      return node.point;
    })));
  ...
  var onUndo = function onUndo() {
    ...
    nodes.map(function (mn) {
      return mn.node && mn.node.node$.remove();
    });
    nodes = exports.toMetaNodes(JSON.parse(history.pop()));
    render();
  }; // go
...
},{"./dom":"path-edit/dom.ts","./dragger":"path-edit/dragger.ts","./snapper":"path-edit/snapper.ts","../point-maths":"point-maths.ts"}],"demo/undo-redo.ts":[function(require,module,exports) {
"use strict";
...
},{"./undo-redo":"demo/undo-redo.ts"}],"demo-curvy-v2.ts":[function(require,module,exports) {
"use strict";
...
var mod_1 = require("./path-edit/mod");
...
},{"./demo.css":"demo.css","./path-edit/mod":"path-edit/mod.ts","./demo/handle-key-events":"demo/handle-key-events.ts","d3":"../node_modules/d3/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","demo-curvy-v2.ts"], null)
//# sourceMappingURL=/demo-curvy-v2.9538094c.js.map